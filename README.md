...
- Output:
  - A set of **feasible, Pareto-optimal configurations**  
    \[
    \{ y_1, y_2, \dots, y_K \}
    \]
...
\[
\mathrm{VaR}_{\alpha}(Z)
= \inf \left\{ z \in \mathbb{R} : \mathbb{P}(Z \le z) \ge \alpha \right\}
\]

- VaR is **configuration-specific**
- Each configuration \( y \) induces a different cost distribution \( Z(y, W) \)
...
= \mathbb{E}\!\left[ Z \mid Z \ge \mathrm{VaR}_{\alpha}(Z) \right]
\]
...
The final configuration \( y^* \) is selected by:

- **Primary criterion:**  
  \[
  \min_y \ \mathrm{CVaR}_{0.9}(Z(y))
  \]
...
- Uses the **same demand distribution** throughout (avoids re-estimation noise)
...
```bash
python robust_supply_chain_monte_carlo.py
